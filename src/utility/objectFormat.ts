...
const formatEntry = (data: any) => {

  const formattedTitle = formatTitle(capitalizeKey(data.key));
  const formattedBody = Array.isArray(data.value) ? formatArray(data.value) : data.value;
